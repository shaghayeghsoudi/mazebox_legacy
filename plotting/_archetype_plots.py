...
def _archetype_diagram_with_arrows(_adata, score_name,subtypes, color, figsize, n_types, color_dict, order,multiplier, g = 'All',cmap = 'viridis',
    ...
    if type(colors[0]) == str:
        ...
        c = colors
        if mean_only:
            plt.scatter(X_df_data[0].mean(), X_df_data[1].mean(), c=c.mean(), zorder=3, alpha=alpha, s=s)
    ...
    if grid:
        ...
        plt.quiver(binned_grid['binned_x'].values,binned_grid['binned_y'].values,binned_grid['u'].values,binned_grid['v'].values,
                   angles='xy', scale_units='xy', zorder = 4, headlength=2,headaxislength=2,
                   headwidth=3,scale=scale,alpha=alpha,width=0.003,linestyle='solid')
...
def subtype_bar(adata, groupby, fname, pheno_name = 'Phenotype_filtered', custom_palette = custom_palette):
    import matplotlib.pyplot as plt

    plt.rcParams["figure.figsize"] = [8, 4]
    ((pd.crosstab(adata.obs[groupby], adata.obs[pheno_name]).T /
      pd.crosstab(adata.obs[groupby], adata.obs[pheno_name]).sum(axis=1)) * 100).T.plot(kind='bar',
                                                                                                  stacked=True,
                                                                                                  linewidth=0,
...
def twodimplot_tumor(data, veldata, adata, gene_sig,celltype1, celltype2, save=False, onecluster=False,
                      cluster=None, tumor_clust_name = 'louvain', axes = False, meanonly = False, dir_prefix = None, palette = sns.color_palette("Paired")):
    if type(dir_prefix) == type(None) and save != False: print("must define out directory using argument 'dir_prefix' to save figure")
    ...
    if tumor_clust_name != 'louvain':
        ...
        adata.obs[tumor_clust_name] = [tumor_clust_dict[i] for i in list(adata.obs[tumor_clust_name].values)]
    ...
    if onecluster == False:
        if meanonly == True:
            ...
            plt.axhline(y=0, color='gray', alpha=.5, linestyle='--')
            plt.axvline(x=0, color='gray', alpha=.5, linestyle='--')
            if axes != False:
                plt.xlim(axes[0])
                plt.ylim(axes[1])
            else:
                plt.xlim([np.array([data[celltype1].min(), data[celltype2].min()]).min(), np.array([data[celltype1].max(), data[celltype2].max()]).max()])
                plt.ylim([np.array([data[celltype1].min(), data[celltype2].min()]).min(), np.array([data[celltype1].max(), data[celltype2].max()]).max()])

            labels = pd.DataFrame(adata.obs[tumor_clust_name].values, columns=['label'], index=data.index)
            for label in list(set(adata.obs[tumor_clust_name])):
                ...
                plt.annotate("",
                             xy=(data.loc[labels['label'] == label, celltype1].mean() + veldata.loc[labels['label'] == label, celltype1].mean(),
                                 data.loc[labels['label'] == label, celltype2].mean() + veldata.loc[
                                     labels['label'] == label, celltype2].mean()),
                             xytext=(data.loc[labels['label'] == label, [celltype1, celltype2]].mean()),
                         arrowprops=dict(facecolor='black',edgecolor = 'black', width=1, headlength=3, headwidth=3, alpha=1))
            plt.title([clust_names[celltype1], ' vs. ', clust_names[celltype2]])
            plt.tight_layout()
            if save == True:
                plt.savefig(
                    op.join(dir_prefix, f'{clust_names[celltype1]} vs. {clust_names[celltype2]}.png'))
                plt.close()
            elif type(save) == str:
            ...
            plt.scatter(x=data[celltype1],
                        y=data[celltype2], s=10, color=[customPalette[int(i)] for i in adata.obs[tumor_clust_name]],
                        alpha=1)
            if type(veldata) != type(None):
                for x, y in zip(range(len(data[celltype1])), range(len(data[celltype1]))):
                    plt.annotate("",
                                 xy=(data[celltype1][x] + veldata[celltype1][x], data[celltype2][y] + veldata[celltype2][y]),
                                 xytext=(data[celltype1][x], data[celltype2][y]),
                                 arrowprops=dict(facecolor='black', width=1, headlength=3, headwidth=3, alpha=.2))
            plt.axhline(y=0, color='gray', alpha=.5, linestyle='--')
            plt.axvline(x=0, color='gray', alpha=.5, linestyle='--')
            if axes != False:
                plt.xlim(axes[0])
        ...
        if cluster == None:
            print('You must specify a cluster from the test data for onecluster == True. Use argument "cluster."')
        else:
            plt.figure(figsize=(20, 20))

            list_c = (adata.obs[tumor_clust_name] == cluster).values
            print(len(data.loc[list_c,celltype1]))
            plt.scatter(x=data.loc[list_c,celltype1],
                        y=data.loc[list_c,celltype2], s=10,
                        alpha=1)
            plt.axhline(y=0, color='gray', alpha=.5, linestyle='--')
            plt.axvline(x=0, color='gray', alpha=.5, linestyle='--')
            for x, y in zip(data.loc[list_c,celltype1].index.values,
                            data.loc[list_c,celltype1].index.values):
                if veldata != None:
                    plt.annotate("", xy=(data.loc[list_c,celltype1][x] +
                                         veldata.loc[list_c,celltype1][x],
                                         data.loc[list_c, celltype2][y] +
                                         veldata.loc[list_c, celltype2][y]),
                                 xytext=(data.loc[list_c,celltype1][x],
                                         data.loc[list_c, celltype2][y]),
                                 arrowprops=dict(facecolor='black', width=1, headlength=5, headwidth=5, alpha=.2))
