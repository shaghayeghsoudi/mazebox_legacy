...
def ctrp_groups(adata, groups, basis = 'umap', ctrp = 'ctrp', vmax = None):
    for r in sorted(adata.obs[groups].cat.categories):
        _adata = adata.copy()
        _adata = _adata[_adata.obs[groups] == r,]
        ax = scv.pl.scatter(adata, color='lightgrey', show=False, alpha=1, size=100, basis=basis)

        scv.pl.scatter(_adata, color=ctrp, basis=basis, fontsize=24, size=40, vmin=0, vmax = vmax,
                       color_map='jet', title=r, alpha=0.5, ax=ax, zorder=2, legend_loc='right margin')

def ctrp_hist(adata, groups = None, bins = 100, fname = None,ctrp = 'ctrp', figsize = (8,6), custom_palette = sns.color_palette("muted")
):
