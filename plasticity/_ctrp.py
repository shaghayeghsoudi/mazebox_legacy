...
def calculate_ctrp(N, D):
    fate = (N.T / N.sum(axis=1)).T  # each row is a different starting cell; fate is proportion of time spent in each state j when starting from i
...
def ctrp_simplified(data, vkey='velocity', groupby=None, groups=None, self_transitions=True, basis=None, eps_eig = 1e-3,
                    weight_diffusion=0, scale_diffusion=1, eps=1e-2, copy=False, distance_basis = 'X_pca', random_seed = 1):
    ...
    for c, cat in enumerate(categories):
        ...
        Q, ends = extract_Q(T, connectivities = connectivities, eigen=True,eps = eps, eps_eig = eps_eig)
