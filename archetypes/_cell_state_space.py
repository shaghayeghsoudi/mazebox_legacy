...
def read_ref(file, cells = False, genes = False, ref_states = False, delim = '\t', transpose = False, unlog = False, normalize = False):
    print("WARNING: read_ref assumes that data is in TPM format (or log TPM). "
    ...
    if type(ref_states) != bool:
        ...
        if type(ref_states) == pd.DataFrame or type(ref_states) == pd.Series:
            if type(ref_states.index[1]) == np.int64:
                print("Changing ref_states indices to string to match ref_map obs_names...")
    ...
    if type(normalize) != bool:
        if normalize == 'scale':
            if transpose == True:
                ...
                data.X = (data.X- data.X.min()) / (data.X.max() - data.X.min()) #assumes cells are columns
...
def genesig_rank_groups(ref_map, ref_states, n_genes = 4, state_id = 'State'):
    print(f"Creating gene signature using rank_gene_groups in Scanpy with {n_genes} genes...")
    ...
    gene_sig = pd.DataFrame(0, index=list(set(ref_map.obs[state_id])), columns=data.columns)
...
def transform_ref_space(ref_map, gene_sig, ref_state_df, norm = False, scale = False):
    print("Shape of b (ref data) is: ", ref_map.shape, " and shape of A (gene sig matrix) is: ", gene_sig.shape)
    print("Solving equation Ax = b...")
    ref_map.X = (ref_map.X.T/np.linalg.norm(ref_map.X, axis = 1)).T
    df_inv, resid, rank, sing_values = np.linalg.lstsq(gene_sig, ref_map.X.T, rcond=1) #FutureWarning without rcond statement
    cells = pd.DataFrame((df_inv.T[:, 0:len(set(ref_state_df['cluster']))]), index=ref_state_df.index)
    if norm == True:
...
def transform_vel(tumor, gene_sig, norm = False, scale = False, scaling = None, normalize = False, lanorm = None):
    ...
    if type(lanorm) != type(None):
        ...
        tumor.layers["velocity"] = (tumor.layers["velocity"].T/np.linalg.norm(tumor.layers["velocity"], axis = 1)).T

    data_vel = pd.DataFrame(tumor.layers['velocity'], columns=tumor.var_names, index=tumor.obs_names)
    if type(normalize) != bool:
        if normalize == 'scale':
